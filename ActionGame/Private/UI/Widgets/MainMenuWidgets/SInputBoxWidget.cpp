...
FReply SInputBoxWidget::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	...
	if (bCanChangeText && Key != PreKey && Key != EKeys::Escape)
	{
		...
		{
			const TArray<FInputActionKeyMapping>& Mappings = UPlayerInput::GetEngineDefinedActionMappings();
			It->RemoveActionMapping(FInputActionKeyMapping(*InputMappingName));
		}*/
